...
class ComputerDAO(db: Database) {
   ...
   */
  val withCompany = this.simple ~ (Company.simple ?) map {
  ...
  def findById(id: Long): Option[Computer] = {
    db.withConnection { implicit connection =>
      SQL("select * from computer where id = {id}").on('id -> id).as(this.simple.singleOpt)
    }
  }
